...
const EdgeBlackjack: React.FC = () => {
  ...
  const [currentGame, setCurrentGame] = useState<{
    ...
    dealerHand: Card[]
    deck: Card[]
    gameState: 'betting' | 'playing' | 'dealer' | 'complete'
    splitHands?: Card[][]
  ...
  // Shuffle deck using provably fair system
  const shuffleDeck = (deck: Card[]): Card[] => {
    const shuffled = [...deck]
    const seed = session.serverSeed + session.clientSeed + session.nonce
    
    // Use a more robust provably fair shuffle
  ...
  const updateAvailableActions = () => {
    ...
    const canDouble = hand.length === 2 && !currentGame.splitHands
    const canSplit = canSplitCards(hand) && !currentGame.splitHands
  ...
  const playDealerHand = async () => {
    ...
    if (isBust(newDealerHand)) {
      await handleGameEnd('dealer_bust')
    } else {
  ...
  // Handle game end
  const handleGameEnd = async (outcome?: 'blackjack' | 'dealer_blackjack' | 'push' | 'bust' | 'dealer_bust' | 'win' | 'loss') => {
    ...
    if (!outcome) {
      if (isBlackjack(currentGame.playerHand)) {
        ...
        outcome = 'bust'
      } else if (isBust(currentGame.dealerHand)) {
        outcome = 'dealer_bust'
    ...
    return (
      <div className="text-center">
        ...
        <div className="text-lg font-bold text-[#00d4ff]">
          {hideSecond ? calculateHandValue([hand[0]]) : value}
        </div>
      </div>
    )
  }
        ...
        {currentGame.splitHands.map((hand, index) => (
          <div key={index} className="text-center">
            <h3 className="text-lg font-semibold text-white mb-2">
              Hand {index + 1} {currentGame.currentHandIndex === index ? '(Current)' : ''}
            </h3>
            <div className="flex justify-center space-x-2 mb-2">
